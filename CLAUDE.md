...
- `client/`: `capture.py` (`CameraCapture` protocol + `OpenCVCapture` implementation), `transport.py` (`ClientSession` — connects to the server, sends frames, matches detections, handles reconnect), `__main__.py` (entrypoint wiring config, camera, transport, zone analysis, and debounce)
